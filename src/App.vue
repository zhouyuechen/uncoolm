...
<mt-navbar  class="bg  " v-model="selected">
    ...
    <!-- 搜索按钮 -->
    <div :class="search">
      <input type="search" class="sea_a"  @focus="showSearch" @touchend ="showSearch"  :placeholder="zhanwei" v-model="search_value"  />
      <button  @touchend="letsSearch" >🔍</button>
    </div>
    <!-- 组件显示区 -->
    <router-view v-on:jump="changeS" v-on:search_val="change_search_val"  
    v-on:playThis="getsong2"  v-on:stop="playmusic" 
     :res="search_res" style="margin-bottom:4rem"  />
      <!-- 返回顶部按钮 -->
      <button :class="back_btn" @touchstart="backTop"  >TOP</button>
<!-- 底部播放器 -->
    <div class="play_box">
      <div class="play_pic"   @touchend="showSong"   ><img :src="picurl" alt="加载失败"></div>
    ...
    <div @touchmove="handleTouchmove" class="warp" style="overflow-y:scroll">
      <mt-popup v-model="popupVisible" lockScroll=true position="bottom" class="mint-popup" :modal="false">
     <div class="back">
     <mt-button @touchend.native="back"   size="large" type="primary">返回</mt-button><p v-text="songname"></p></div>
...
export default {
  ...
  methods: {
    ...
    _currentTime: function () {  setTimeout(() => {
         const self = this;     
    ...
    playmusic(val){
      ...
      }else if(val==77){/* this.$refs.player.play(); */
         this.play_img="btn bg_s";
      }
        else
     {if(this.$refs.player.paused)
...
#app {
  ...
  .mint-popup{
    ...
    img.re{
      ...
      display: block;
      margin: 1rem auto;
      border: .5rem solid rgba($color: #666, $alpha: .7);
