...
<template>
 <div class="look">
  ...
  <div @touchmove="handleTouchmove" class="warp" style="overflow-y:scroll">
   ...
   >
    <div class="back">
     <mt-button @click.native="back" size="large" type="primary">返回</mt-button>
     <span>{{this.mvDetails.name}}</span>
    </div>

    <div v-if="mvDetails.cover" class="video">
     <video
      class="video_self"
      :src="mvTrueUrl"
      ref="video"
     ...
     ></video>
     <div class="info1">
      <p>{{mvDetails.artists[0].name}}</p>
      <p>播放次数：{{mvDetails.playCount}}</p>
     </div>
     <!-- 其他mv -->
     <p>相似mv</p>
    ...
    <!--  <div class="info1">
      ...
      <p>发布时间：{{mvDetails.publishTime}}</p>
     </div> -->
    ...
    <!--  <ul class="comment" v-if="comments[1]">
      <li v-for="(item,i) in comments" :key="i">
       <div class="com1">
        <img :src="item.user.avatarUrl" alt="加载失败">
  ...
  loadMore() {
   ...
   setTimeout(() => {
    ...
    if (last == this.res.length) {
     this.loading = true;
     Toast({ message: "已经到底啦·", duration: 2000, className: "toast" });
     return;
    }
